...
fn bench_delete_all(c: &mut Criterion) {
    ...
    for size in [100, 1000, 10000] {
        // Smaller sizes for delete since it's destructive
        let keys = generate_random_keys(size, 12345);
...
fn bench_memory_analysis(c: &mut Criterion) {
    ...
    for size in [1000, 10000, 100000] {
        ...
        group.bench_with_input(
            BenchmarkId::new("miso_memory_per_element", size),
            &keys,
            |b, keys| {
                b.iter(|| {
                    let mut map = Miso::with_capacity(size);
                    ...
                    let memory_per_element = total_memory / map.len();
        ...
        group.bench_with_input(
            BenchmarkId::new("std_memory_per_element", size),
            &keys,
            |b, keys| {
                b.iter(|| {
                    let mut map = HashMap::with_capacity(size);
